...
```bash
docker buildx build --rm --load -t cutsea110/sched:0.5.1 .
```

## Docker で実行

```bash
docker run -e TZ=Asia/Tokyo -v ${PWD}:/work --user "$(id -u):$(id -g)" -it --rm cutsea110/sched:0.5.1 -o /work/sheet
```
...
```bash
docker login
docker push cutsea110/sched:0.5.1
```
