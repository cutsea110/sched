...
```bash
docker buildx build --rm --load -t cutsea110/sched:0.6.7 .
```

## Docker で実行

```bash
docker run -v ${PWD}:/work --user "$(id -u):$(id -g)" -it --rm cutsea110/sched:0.6.7 -o /work/sheet.tex
```
...
```bash
docker login
docker push cutsea110/sched:0.6.7
```
